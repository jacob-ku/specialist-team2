...
void FaceNetClassifier::addNewFace(cv::Mat &image, std::vector<struct Bbox> outputBbox) {
    ...
    std::vector<uchar> sendbuff;//buffer for coding

    unsigned int imagesize;
    cv::imencode(".jpg", image, sendbuff, param);
    encrypt_data(sendbuff.data(), sendbuff.size(), dirPath.c_str(), dirPath.length(), newName.c_str(), newName.length());
}

void FaceNetClassifier::addNewFaceWithName(cv::Mat &image, std::vector<struct Bbox> outputBbox, const std::string name) {
