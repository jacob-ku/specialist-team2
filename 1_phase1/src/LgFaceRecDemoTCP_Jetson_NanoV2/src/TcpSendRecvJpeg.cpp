...
int SendImageAsJpeg(gssSocketClass *pSocket, cv::Mat Image)
{
    unsigned int imagesize;
    ...
    if (pSocket->sendData((const unsigned char *)&imagesize, sizeof(imagesize)) != sizeof(imagesize)) return (-1);
    return(pSocket->sendData((const unsigned char *)sendbuff.data(), sendbuff.size()));
}
