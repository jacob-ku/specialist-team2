...
int TcpSendImageAsJpeg(TTcpConnectedPort* TcpConnectedPort, cv::Mat Image)
{
	...
	imagesize = htonl((u_long)(sendbuff.size())); // convert image size to network format
	if (WriteDataTcp(TcpConnectedPort, (unsigned char*)&imagesize, sizeof(imagesize)) != sizeof(imagesize))
		return(-1);
	return((int)WriteDataTcp(TcpConnectedPort, sendbuff.data(), sendbuff.size()));
}
...
bool TlsRecvImageAsJpeg(SSL* TcpConnectedPort, cv::Mat* Image)
{
	unsigned int imagesize;
	unsigned char* buff;	/* receive buffer */

	if (ReadDataTls(TcpConnectedPort, (unsigned char*)&imagesize, sizeof(imagesize)) != sizeof(imagesize)) return(false);
	...
	if ((ReadDataTls(TcpConnectedPort, buff, imagesize)) == imagesize)
	{
		cv::imdecode(cv::Mat(imagesize, 1, CV_8UC1, buff), cv::IMREAD_COLOR, Image);
		delete[] buff;
		if (!(*Image).empty()) return true;
